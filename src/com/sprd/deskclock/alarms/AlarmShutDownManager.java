...
public class AlarmShutDownManager {
    ...
    private static File ALARM_FLAG_FILE = new File("/mnt/vendor/alarm_flag");

    public static Handler mWriteShutDownFileHandler;

    public static volatile HandlerThread mWriteShutDownFileHandlerThread;
    ...
    public static void alarmFlagSetup(Context context,AlarmInstance alarm) {
        ...
        try {
            Runtime.getRuntime().exec("chmod 664 /mnt/vendor/alarm_flag");
    ...
    public static void createOrUpdateShutDownFile(final Context context,
        ...
        int powerOffType;
        if (isPowerOff) {
            powerOffType = AlarmManagerWrapper.POWER_OFF_WAKEUP;
        } else {
            powerOffType = AlarmManagerWrapper.POWER_OFF_ALARM;
        }
        if (isKitKatOrLater) {

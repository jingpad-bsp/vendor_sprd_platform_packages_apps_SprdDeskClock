...
public class TouchInterceptor extends ListView {
        ...
        if (mDragListener != null || mDropListener != null) {
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    mTouchPointX = (int) ev.getX();
                    mTouchPointY = (int) ev.getY();
                    itemnum = pointToPosition(mTouchPointX, mTouchPointY);
                    if (itemnum == AdapterView.INVALID_POSITION) {
                        break;
                    }
                    item = (ViewGroup) getChildAt(itemnum - getFirstVisiblePosition());
                    mDragPointX = mTouchPointX - item.getLeft();
                    mDragPointY = mTouchPointY - item.getTop();
                    mXOffset = ((int)ev.getRawX()) - mTouchPointX;
                    mYOffset = ((int)ev.getRawY()) - mTouchPointY;
                    /* UNISOC: Modify for bug1165185  The left/right side of the item is the grabber for dragging the item @{ */
                    int width = BitmapFactory.decodeResource(this.getResources(), R.drawable.citylist_tile_drag).getWidth();
                    if ((isRtl() && mTouchPointX > item.getWidth() - width) || !isRtl() && mTouchPointX < width) {
                        creatDrawingCopy();
                        return false;
                    }
                    /* @} */
    ...
    private void startDragging(Bitmap bm, int x, int y) {
        ...
        /* UNISOC: Modify for bug1152076  Make a copy of the drawing close to the left side of the root View avoiding the impact of the virtual navigation bar @{ */
        mWindowParams.x = 0;
        /* @} */
        mWindowParams.y = y - mDragPointY + mYOffset;

        mWindowParams.height = WindowManager.LayoutParams.WRAP_CONTENT;
        /* UNISOC: Modify for bug1152076 @{ */
    ...
    /* UNISOC: Modify for bug1165185 @{ */
    private boolean isRtl() {
        return TextUtils.getLayoutDirectionFromLocale(Locale.getDefault()) == View.LAYOUT_DIRECTION_RTL;
    }

    private void creatDrawingCopy() {
        item.setDrawingCacheEnabled(true);
        // Create a copy of the drawing cache so that it does not get recycled
        // by the framework when the list tries to clean up memory
        Bitmap bitmap = Bitmap.createBitmap(item.getDrawingCache());
        item.destroyDrawingCache();
        startDragging(bitmap, mTouchPointX, mTouchPointY);
        mDragPos = itemnum;
        mSrcDragPos = mDragPos;
        mHeight = getHeight();
        int touchSlop = mTouchSlop;
        mUpperBound = Math.min(mTouchPointY - touchSlop, mHeight / 3);
        mLowerBound = Math.max(mTouchPointY + touchSlop, mHeight * 2 /3);
    }
    /* @} */

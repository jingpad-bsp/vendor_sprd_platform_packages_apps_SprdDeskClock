...
final class SettingsDAO {
    ...
    static Weekdays.Order getWeekdayOrder(SharedPreferences prefs) {
        ...
        /* UNISOC: Modify for bug 1404759 {@ */
        String value = prefs.getString(SettingsActivity.KEY_WEEK_START, defaultValue);
        if (Integer.parseInt(value) == Calendar.FRIDAY) {
            value = defaultValue;
            prefs.edit().putString(SettingsActivity.KEY_WEEK_START, value).apply();
            LogUtils.i("Adapt OTA upgrade from 9.0 to 10.0, current first day of week is " + value);
        }
        /* @} */

...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    public static AlarmInstance mForVolumeInstance = null;
    ...
    public static void setFiredState(Context context, AlarmInstance instance) {
         ...
         * SPRD: bug 473507 add auto silences for each alarm @{
         *
         * @orig Calendar timeout = instance.getTimeout(context);
         */
        /* @} */
        Calendar timeout;
    ...
    public static void setPreDismissState(Context context, AlarmInstance instance) {
        ...
        // SPRD: bug 474360 when click "Dismiss now" or dismiss alarm in alarm notification, poweron off alarm can still work
     ...
     */
       public static void registerInstance(Context context, AlarmInstance instance,
         ...
         * SPRD: bug 473507 add auto silences for each alarm @{
         *
         * @orig final Calendar timeoutTime = instance.getTimeout(context);
         */
        /* @} */
        ...
        if (AplicationSupportChange.SUPPORT_ALARM_ALERT_LONG) {
            ...
            timeoutTime = instance.getTimeout();
        }
    ...
    public static void fixAlarmInstances(Context context) {
        ...
        for (AlarmInstance instance : AlarmInstance.getInstances(contentResolver, null)) {
            ...
            if (currentTime.before(priorAlarmTime) || currentTime.after(missedTTLTime)) {
                ...
                /* UNISOC:: Modify for bug 1119096 @{ */
                if (newAlarmTime.before(oldAlarmTime)) {
                    instance.setAlarmTime(newAlarmTime);
                    AlarmInstance.updateInstance(contentResolver, instance);
                }
                /* @} */
    ...
    public static void handleIntent(Context context, Intent intent) {
        ...
        if (CHANGE_STATE_ACTION.equals(action)) {
            ...
            /* SPRD: Bug 515840,591844 when playing video, alarm will not ring after arrival @{ */
            if (context.getResources().getBoolean(R.bool.video_paused_at_fired_state)
                         && AlarmInstance.FIRED_STATE == alarmState && isLiveMoveTopActivity(context)) {
            ...
            /*
             * SPRD bug 604729 when alarm ring, press volume button, alarm will
             * not off or pause @{
             */
            ...
            deleteInstanceAndUpdateParent(context, instance);
        /* SPRD bug 604729 when alarm ring, press volume button, alarm will not off or pause @{ */
        } else if (intent.getAction() != null && intent.getAction().equals("com.sprd.deskclock.volume_behavior")) {
            if (mForVolumeInstance == null) {
                LogUtils.e("mForVolumeInstance = null ");
                return;
            }
            String mVolumeBehavior = Utils.getDefaultSharedPreferences(context)
                    .getString(SettingsActivity.KEY_VOLUME_BUTTONS,
                            SettingsActivity.DEFAULT_VOLUME_BEHAVIOR);
            if (mVolumeBehavior != null) {
                ...
                case SettingsActivity.VOLUME_BEHAVIOR_DISMISS:
                    AlarmStateManager.deleteInstanceAndUpdateParent(context,
                            mForVolumeInstance);
                    break;
                default:
                    break;
        ...
        /* SPRD: Bug 515840,591844 when playing video, alarm will not ring after arrival @{ */
            ...
            if (mForLiveInstance == null) {
                Log.e("AlarmStateManager",
                        "handleIntent()---mForLiveInstance = null");
                return;
            }
            Intent newIntent = AlarmInstance.createIntent(context, AlarmService.class, mForLiveInstance.mId);
            newIntent.setAction(CHANGE_STATE_ACTION);
            newIntent.addCategory(ALARM_MANAGER_TAG);
            newIntent.putExtra(ALARM_GLOBAL_ID_EXTRA, getGlobalIntentId(context));
            newIntent.putExtra(ALARM_STATE_EXTRA, AlarmInstance.FIRED_STATE);
    ...
    private static class AlarmManagerStateChangeScheduler implements StateChangeScheduler {
        ...
        public void scheduleInstanceStateChange(Context context, Calendar time,
            ...
            /* SPRD: bug 474360 add poweronoff alarm @{ */
            LogUtils.v("context.getUserId() = " + context.getUserId());
            // SPRD: bug 621218 add judgement for UserId
            if (context != null && context.getUserId() == 0) {
                AlarmShutDownManager.createOrUpdateShutDownFile(context,
                        instance, am, newState, false);
            }
            /* @} */
        }
        ...
        public void cancelScheduledInstanceStateChange(Context context, AlarmInstance instance) {
            ...
            if (pendingIntent != null) {
                 ...
                 * */
                AlarmManagerWrapper.cancelAlarm(am, pendingIntent, null);
    ...
    public static void updatePowerOffAlarm(Context context, AlarmInstance instance) {
        /* UNISOC:: Modify for bug 1148923 @{ */
        final AlarmInstance nextAlarm = getNextFiringAlarm(context);
        final AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
        if (nextAlarm != null) {
            AlarmShutDownManager.createOrUpdateShutDownFile(context, nextAlarm);
            AlarmShutDownManager.createOrUpdateShutDownFile(context, nextAlarm, am, nextAlarm.mAlarmState, true);
        }
        /* @} */
    }

...
public final class MoveScreensaverRunnable implements Runnable {
    ...
    /* UNISOC: Modify for bug1151409 define safe area distance screen distance variable @{ */
    private float mSafeInsetLeft;
    private float mSafeInsetRight;
    private float mSafeInsetTop;
    private float mSafeInsetBottom;
    /* @} */
    ...
    public void run() {
        ...
        /* UNISOC: Modify for bug1151409 get the safe area distance screen distance variable @{ */
        WindowInsets windowInsets = mContentView.getRootWindowInsets();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && windowInsets != null) {
            DisplayCutout displayCutout = windowInsets.getDisplayCutout();
            if (displayCutout != null) {
                mSafeInsetLeft = (float) displayCutout.getSafeInsetLeft();
                mSafeInsetRight = (float)displayCutout.getSafeInsetRight();
                mSafeInsetTop = (float)displayCutout.getSafeInsetTop();
                mSafeInsetBottom = (float)displayCutout.getSafeInsetBottom();
            }
        }
        /* @} */
        if (selectInitialPosition) {
            ...
            /* UNISOC: Modify for bug1151409  @{ */
            mSaverView.setX(newX + mSafeInsetLeft);
            mSaverView.setY(newY + mSafeInsetTop);
            /* @} */
            ...
            /* UNISOC: Modify for bug1151409  @{ */
            final float newX = getRandomPoint(mContentView.getWidth() - mSaverView.getWidth() - mSafeInsetRight - mSafeInsetLeft);
            final float newY = getRandomPoint(mContentView.getHeight() - mSaverView.getHeight() - mSafeInsetBottom - mSafeInsetTop);
            /* @} */

...
public class AnalogClock extends FrameLayout {
    ...
    /* UNISOC: Modify for bug 1405093 {@ */
    private void onTimeChanged() {
        mTime.setTimeInMillis(System.currentTimeMillis());
        final float second = mTime.get(Calendar.SECOND);
        final float minute = mTime.get(Calendar.MINUTE) + second / 60f;
        final float hour = mTime.get(Calendar.HOUR) + minute / 60f;
        LogUtils.d("The current time is " + DateFormat.format(mDescFormat, mTime));
        LogUtils.d("hour is " + hour + ", minute is " + minute + ", second is " + second);

        final float hourAngle = hour * 30f;
        mHourHand.setRotation(hourAngle);
        final float minuteAngle = minute * 6f;
        mMinuteHand.setRotation(minuteAngle);
        if (mEnableSeconds) {
            final float secondAngle = second * 6f;

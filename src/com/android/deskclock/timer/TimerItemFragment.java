...
public class TimerItemFragment extends Fragment {
    ...
    boolean updateTime() {
        ...
        if (view != null) {
            ...
            if (timer.isRunning() && timer.getTotalLength() > 0 && timer.getRemainingTime() < 0) {
                getActivity().startService(TimerService.createTimerExpiredIntent(getActivity(), timer.expire()));
            }
            view.update(timer);
